...
package body Interpreter is
   ...
   begin
      ...
      Delete_Last (Stack);
      Check_Push_NT (Stack, A.Val, B.Val, 1, 0, Op);
   end Execute_Compare;
   ...
   procedure Calculate_Push_Unsigned
     (Stack : in out Vector; A, B : U; Op : Arithmetic)
   is
      Result : U;
   begin
      Result := Calculate_Unsigned (A, B, Op);
      Push_Unsigned (Stack, Result);
   end Calculate_Push_Unsigned;
   ...
   is
      Result : F;
   begin
      Result := Calculate_Float (A, B, Op);
      Push_Float (Stack, Result);
   end Calculate_Push_Float;

   function Clc_S
     (A, B : Signed_Type; Op : Arithmetic; Size : Signed) return Signed_Type
   is
   begin
      case Size is
         when I_32 =>
   ...
   procedure Push_S (Stack : in out Vector; Result : Signed_Type) is
   begin
      Stack.Append
        ((Value,
   ...
   ---------------------------------
   procedure Calculate_S32 is new Calculate_Push_Signed
     (S => Signed_Type, Calculate_Signed => Clc_S, Push_Signed => Push_S);

   procedure Execute_Arithmetic
     (Stack : in out Vector; T : S_F; Op : Arithmetic)
   is
   ...
   end Execute_Arithmetic;

   --  procedure Execute_Addition (Stack : in out Vector; Op : Arithmetic) is
   --     A, B   : Element_Variation;
   --     Result : Integer_32;
   --  begin
   --     A := Last_Element (Stack);
