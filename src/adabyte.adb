...
begin
   ...
   Append (Locals_func, (Number_Value, Num_Value => (I_32, I_32 => 0)));

   --  one params a = 1
   Append (Params, (Number_Value, Num_Value => (I_32, I_32 => 1)));

   --  one var to return $n (only need to specify the type of the result)
   Append (Results, (Number_Value, Num_Value => (I_32, I_32 => 0)));
